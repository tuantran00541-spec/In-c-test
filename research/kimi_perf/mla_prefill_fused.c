...
/* Research copy of the established low-memory MLA prefill. The only semantic
 * change is ownership of latent/rope storage: instead of allocating temporary
 * arrays and then recomputing kv_a_proj/RMSNorm/RoPE in a second pass, this
 * function writes those exact intermediates directly into the persistent
 * compressed state used by decode. */

static void rope_interleaved_fused(float *dst, const float *raw, int dim,
...
static double dot_f32_f64_fused(const float *a, const float *b, int n) {
#ifdef KVL_USE_AVX2
...
static void value_add_f64_fused(double *acc, const float *v, int n, double p) {
#ifdef KVL_USE_AVX2
...
int kvl_mla_prefill_compressed_fused_bf16(float *out,
    ...
    if (!k_nope || !v_states || !scores_all || !qtmp_all || !katmp || !kvtmp ||
        !qrope_all || !head_tmp || !value_acc_all || !head_seq) {
        free(k_nope); free(v_states); free(scores_all); free(qtmp_all);
        free(katmp); free(kvtmp); free(qrope_all); free(head_tmp);
        free(head_states); free(value_acc_all);
        return -1;
    }

    /* This is the pass that production previously performed twice. The values
     * are written directly into persistent decode state on the first pass. */
    for (int t = 0; t < S; ++t) {
        ...
        rope_interleaved_fused(rope, katmp + R, DR, t, cfg->rope_theta);
    }
...
#pragma omp parallel for schedule(dynamic, 8) if(S >= 128) num_threads(worker_count)
        ...
        for (t = 0; t < S; ++t) {
            ...
            rope_interleaved_fused(qrope, qtmp + DN, DR, t, cfg->rope_theta);

            float max_score = -INFINITY;
            for (int j = 0; j <= t; ++j) {
                const float *kj = k_nope + (size_t)j * DN;
                const float *rj = rope_states + (size_t)j * DR;
                const double dot = dot_f32_f64_fused(qtmp, kj, DN) +
                                   dot_f32_f64_fused(qrope, rj, DR);
            ...
            for (int j = 0; j <= t; ++j) {
                ...
                value_add_f64_fused(value_acc, vj, DV, p);
            }
