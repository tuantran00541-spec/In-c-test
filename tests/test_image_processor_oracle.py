...
def main() -> None:
    ...
    ours, gh, gw = preprocess_image(args.runtime_dir, image)
    ...
    assert ours.shape == ref_patches.shape, (ours.shape, ref_patches.shape)
    d = np.abs(ours - ref_patches)
    rms = float(np.sqrt(np.mean((ours - ref_patches) ** 2)))
