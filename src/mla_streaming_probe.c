...
static int run_case(int seq_len) {
    enum { H = 16, NH = 2, DN = 4, DR = 4, DV = 8, R = 6 };
    ...
    fill_bf16(o, (size_t)H * (NH * DV), 0.20f);
    ...
    printf("mla_streaming_probe seq=%d max_abs=%.9g rel_rms=%.9g\n",
           seq_len, max_abs, rel_rms);
...
int main(void) {
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        ...
        if (rc != 0) {
            fprintf(stderr, "mla_streaming_probe FAILED seq=%d rc=%d\n", cases[i], rc);
            return 1;
        }
    }
