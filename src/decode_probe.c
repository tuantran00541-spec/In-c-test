...
static int layer_attention_prefill(KvlTrunkStore *ts,int L,const float *x,float *r1,float *n2){
    ...
    for(int t=0;t<S;++t)kvl_rmsnorm_bf16(n1+(size_t)t*H,x+(size_t)t*H,(const uint16_t*)in.data,H,RMS_EPS);
    KvlMlaConfig c={H,NH,DN,DR,DV,R,RMS_EPS,ROPE_THETA};
    ...
    for(int t=0;t<S;++t){for(int i=0;i<H;++i)r1[(size_t)t*H+i]=x[(size_t)t*H+i]+a[(size_t)t*H+i];kvl_rmsnorm_bf16(n2+(size_t)t*H,r1+(size_t)t*H,(const uint16_t*)pn.data,H,RMS_EPS);}
    rc=0;
...
static int layer_attention_decode(KvlTrunkStore *ts,int L,const float *x,int pos,KvlMlaCompressedState *state,float *r1,float *n2){
    ...
    kvl_rmsnorm_bf16(n1,x,(const uint16_t*)in.data,H,RMS_EPS);KvlMlaConfig c={H,NH,DN,DR,DV,R,RMS_EPS,ROPE_THETA};KvlMlaBF16 w={(const uint16_t*)q.data,(const uint16_t*)kva.data,(const uint16_t*)kvan.data,(const uint16_t*)kvb.data,(const uint16_t*)o.data};
    if(kvl_mla_decode_compressed_bf16(a,n1,pos,&c,&w,state)!=0)goto done;
    for(int i=0;i<H;++i)r1[i]=x[i]+a[i];kvl_rmsnorm_bf16(n2,r1,(const uint16_t*)pn.data,H,RMS_EPS);rc=0;
...
int main(int argc,char **argv){
    ...
    KvlMlaCompressedState states[LN];KvlMlaConfig mc={H,NH,DN,DR,DV,R,RMS_EPS,ROPE_THETA};size_t state_bytes=0;for(int L=0;L<LN;++L){if(kvl_mla_compressed_state_init(&states[L],&mc,S)!=0)return 1;state_bytes+=kvl_mla_compressed_state_bytes(&states[L]);}
    ...
    KvlTrunkTensor fn={0},lm={0};if(load_kind(&ts,KVL_TRUNK_GLOBAL_LAYER,KVL_TENSOR_FINAL_NORM,&fn)||load_kind(&ts,KVL_TRUNK_GLOBAL_LAYER,KVL_TENSOR_LM_HEAD,&lm))return 1;float *zpf=malloc((size_t)H*sizeof(float)),*zinc=malloc((size_t)H*sizeof(float));kvl_rmsnorm_bf16(zpf,pf+(size_t)(S-1)*H,(const uint16_t*)fn.data,H,RMS_EPS);kvl_rmsnorm_bf16(zinc,x,(const uint16_t*)fn.data,H,RMS_EPS);float *lpf=malloc((size_t)V*sizeof(float)),*linc=malloc((size_t)V*sizeof(float));kvl_matvec_bf16(lpf,zpf,(const uint16_t*)lm.data,H,V);kvl_matvec_bf16(linc,zinc,(const uint16_t*)lm.data,H,V);double ld=maxabs(lpf,linc,V),lr=0;int ap=0,ai=0;for(int i=0;i<V;++i){double d=(double)lpf[i]-linc[i];lr+=d*d;if(lpf[i]>lpf[ap])ap=i;if(linc[i]>linc[ai])ai=i;}lr=sqrt(lr/V);
    printf("tokens=%d rms_eps=%.1e compressed_state_bytes=%zu worst_layer=%d worst_token=%d hidden_max=%.9g logits_max=%.9g logits_rms=%.9g prefill_argmax=%d incremental_argmax=%d trunk_direct_io=%s expert_direct_io=%s\n",S,(double)RMS_EPS,state_bytes,worstL,worstT,worst,ld,lr,ap,ai,ts.direct_io?"yes":"no",es.direct_io?"yes":"no");kvl_expert_cache_report(&cache);
